...
big_integer& big_integer::add_with_shift(big_integer const &other, uint32_t shift) {
    ...
    for(;i < temp.data.size(); i++) {
        buf = r + data[shift + i] + temp.data[i];
        r = buf >> 32;
        data[shift + i] = buf;
    }
...
big_integer::quotient_and_remainder_for_short_unsigned_divisor(uint32_t b) const {
    ...
    int32_t shift = 2;
    a.data.push_back(0);
    uint64_t prefix_a; uint32_t dig_quotient;
    std::vector<uint32_t> quotient_vector;
    int32_t large_word_index = a.data.size() - 1;
    for (large_word_index = a.data.size() - 1; (large_word_index - (shift-1)) >= 0; large_word_index--) {
        prefix_a = a.data[large_word_index];
        prefix_a <<= 32;
        prefix_a += a.data[large_word_index -1];
        dig_quotient = uint32_t(prefix_a / b);
        quotient_vector.push_back(dig_quotient);
        a.add_with_shift(-uint64_to_big_integer(uint64_t(b) * dig_quotient), large_word_index - (shift-1));
...
std::pair<big_integer, big_integer> big_integer::quotient_and_remainder(big_integer const& B) const {
    ...
    if (a == 0) {
        ...
        uint32_t  trial;
        int lll = a.data.size() - b.data.size();
        __uint128_t prefix_a, prefix_b = (__uint128_t(b.data[b.data.size() - 1]) << 32) | b.data[b.data.size() - 2];
        while (lll--) {
            prefix_a = a.data[a.data.size() - 1];
            prefix_a <<= 32;
            prefix_a |= a.data[a.data.size() -2];
            prefix_a <<= 32;
            prefix_a |= a.data[a.data.size() -3];
            prefix_a /= prefix_b;
            trial = uint32_t(((prefix_a >> 32) == 0 ? prefix_a : setted_one));
            // subtraction = b * trial;
            while(a.compare_prefix(b * trial, a.data.size() - len_prefix) < 0) {
                // subtraction -= b;
                trial--;
            }
            quotient_vector.push_back(trial);
            large_word_index = a.data.size() - 1;
            a.add_with_shift(-(b * trial), a.data.size() - len_prefix);
            for(int32_t i = a.data.size()- 1; i > large_word_index; i--) {
                a.data.pop_back();
            }
           if (a.data.back() == 0)
