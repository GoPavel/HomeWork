...
decoder::decoder(uint8_t* code_tree, uint32_t size_code_tree) {
    ...
    for(uint32_t i = 0, j = 0; i < tree.size(); ++i) {
        if(tree[v].l == -1) {
            sym_of_node[v] = lists[++j];
        }
    }
}

vector<uint8_t> decoder::decode_block(uint8_t *code_block, uint32_t bitsize_block, uint32_t byte) {
    vector<uint8_t> decode_block;
    vector<bool> bitcode_block = convert_byte_to_bool(code_block, bit);
    uint32_t v = 0;
