...
/*
*    ## Notes
*    - `end_node` нужен для того, чтобы сет имел нулевое состояние. Что убирает краевые случаи
*    - `end` - это самая большая нода
*    - `const_iterator` не нужен, так как в нем нет особого смысла. Никогда нельзя менять данные по ноде. Поэтому есть только обычный итератор, который возвращает копию объекта.
*    - нельзя хранить предка
*/
